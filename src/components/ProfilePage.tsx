...
export function ProfilePage({ user, onBack, onNavigate, onLogout }) {
  ...
  const menuItems = [
    ...
    { icon: ShoppingCart, label: 'Orders', hasChevron: true, onClick: () => setActiveTab('orders') },
    { icon: HelpCircle, label: 'Support', hasChevron: true, onClick: () => setActiveTab('support') },
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="container-responsive bg-white min-h-screen pb-20">
        ...
        <div className="flex justify-center mt-6 mb-8 gap-4">
          <button
            className={`px-6 py-2 rounded-full font-semibold transition-colors ${activeTab === 'profile' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700'}`}
            onClick={() => setActiveTab('profile')}
          >
            Profile
          </button>
          <button
            className={`px-6 py-2 rounded-full font-semibold transition-colors ${activeTab === 'orders' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700'}`}
            onClick={() => setActiveTab('orders')}
          >
            Order History
          </button>
          <button
            className={`px-6 py-2 rounded-full font-semibold transition-colors ${activeTab === 'support' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700'}`}
        ...
        {/* Tab content */}
        <div className="p-0 lg:p-0 xl:p-0">
          {activeTab === 'profile' && (
            <div className="p-4 lg:p-6 xl:p-8 space-y-6 lg:space-y-8 w-full">
              {/* User Info */}
          ...
          {activeTab === 'orders' && (
            <div className="w-full">
              <OrderHistoryPage user={user} onBack={onBack} onNavigate={onNavigate} />
            </div>
          )}
          {activeTab === 'support' && (
            <div className="w-full">
              <SupportPage user={user} onBack={onBack} onNavigate={onNavigate} />
            </div>
          )}
        </div>
      </div>
